--ARXIU NO ENTREGAT. ÉS PER MI. L'ARXIU D'ENTREGA ES DIU 1_queries_tienda.sql
...
SELECT nombre, precio, precio * 1.17 
FROM producto
...
SELECT p.nombre, p.precio, f.nombre
FROM producto p
JOIN fabricante f  --el producte més barat no té fabricant conegut. Usar LEFT JOIN si es vol el més barat tant si com no. 
ON p.codigo = f.codigo
ORDER BY precio ASC LIMIT 1
...
-- 26-Retorna una llista de tots els productes del fabricador º.
...
WHERE   p.codigo_fabricante =
		...
		FROM   fabricante f
		WHERE  f.nombre = "Lenovo")
ORDER BY precio DESC LIMIT 1 -- Ho hauria fet amb SELECT MAX(p.precio) però no em funciona. Expulsa error 1140 i em diu que hi troba a faltar GROUP BY

-- 39-Llista el nom del producte més barat del fabricant Hewlett-Packard.
SELECT p.nombre, MIN (p.precio)
FROM   producto p
WHERE   p.codigo_fabricante =
		(SELECT codigo
		FROM   fabricante f
...
-- https://www.youtube.com/watch?v=rGPb5E1UAJA&t=266s&ab_channel=pildorasinformaticas
